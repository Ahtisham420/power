...
class UserController extends Controller
{
    ...
    public  function  UserPaymentAll (){
    $posts = UserPayment::with('package','user')->orderBy('id', 'desc')->paginate(8);
    return view('user-payment.index', compact('posts'));
}
  public function UsersPackages(Request $request)
    {
        $posts = UserPackage::with('u_package','user')->orderBy('id', 'desc')->paginate(8);
        return view('users-packages.index', compact('posts'));
    }
...
//        dump($filters);
    ...
    public function status(Request $request)
    {
        ...
        if ($user->update($data)) {
            if ($request->status == User::banned) {
                $body = "<p>Your profile has been banned by the company!.</p>
                   We are sad to inform you that your account has been banned by our company.if this was done by mistake please contact our help center and let us know!.</p>
                    <br><br><p>
                    Regards Power Performance Cars </p>";
                Mail::to($user->email)->send(new defaultNotify("Profile Banned!", $body));
            }
            $response = ['success_msg' => trans('alert.record_updated')];
        } else {
